...
if __name__ == "__main__":
    ...
    for epoch in range(ep):
        ...
        model.eval()
        # TODO
        correct_cnt_v, total_loss_v, total_cnt_v = 0, 0, 0
        for batch, (x, label) in enumerate(val_loader,1):
            ...
            _, pred_label = torch.max(out, 1)
            total_cnt_v += x.size(0)
            correct_cnt_v += (pred_label == label).sum().item()

            # Show the training information
            if batch % 500 == 0 or batch == len(val_loader):
                acc_v = correct_cnt_v / total_cnt_v
                ave_loss_v = total_loss_v / batch
                print ('Training batch index: {}, valid loss: {:.6f}, acc: {:.3f}'.format(
                    batch, ave_loss_v, acc_v))
        model.train()
